...
samples_per_overlap_list = np.unique(np.logspace(3, 5, 10, dtype=int))
num_runs = 10
...
def generate_trace_pool(A, R):
    N = A.shape[0]
    real_part = np.random.choice([1, -1], size=(R, N))
    imag_part = np.random.choice([1, -1], size=(R, N))
    v = real_part + 1j * imag_part
    v /= np.linalg.norm(v, axis=1, keepdims=True)
    Av = v @ A.T
    traces = np.einsum('ij,ij->i', np.conj(v), Av)
    return traces
...
for idx, k in enumerate(k_values):
    ...
    for R in samples_per_overlap_list:
        print(f"Samples per overlap: {R}")
        rmse_runs = []

        # Generate large pools
        pool_size = max(4 * R, 500)
        Z_pool, O_pool = [], []
        for i in M_range:
            U_t = expm(-2j * t_k[i] * H)
            Z_pool.append(generate_trace_pool(U_t, pool_size))
            O_pool.append(generate_trace_pool(U_t @ H, pool_size))

        for _ in range(num_runs):
            z_samp = [np.mean(np.random.choice(z_vals, R, replace=False)) for z_vals in Z_pool]
            o_samp = [np.mean(np.random.choice(o_vals, R, replace=False)) for o_vals in O_pool]
...
plt.legend()
plt.tight_layout()
plt.savefig("sampling_error_fixed_per_overlap_loglog_final.pdf")
plt.show()
