...
# l=0
energies=[]
lambdas=np.linspace(-1.1*np.abs(E_0),1.1*np.abs(E_0),100)
for l in lambdas:
    energies.append(results(l,dt)[-1])

plt.plot(lambdas,energies,color='black',label='$\\langle\\hat{H}\\rangle+\lambda$')
h = 0
for f in H_energies[::2]:

    plt.plot(lambdas, f*np.ones(len(lambdas))+lambdas,linestyle='-.',label='$E_{{{:2d}}}+\\lambda $'.format(h))
    h+=1
plt.ylabel('$E$')
plt.xlabel('$\lambda$')
plt.legend()
...
for f in H_energies[::2]:
    ...
    lines=plt.plot(betas[::2],results(f,dt)-f,label='$\\lambda=E_{{{:2d}}}$'.format(l))

    linecolor=lines[0].get_color()
    plt.plot(betas, -np.ones(len(betas)) *f, linestyle='-.',color=linecolor)
    l+=1
